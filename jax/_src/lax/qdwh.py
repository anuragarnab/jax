...
import jax.scipy.linalg as lsp_linalg
...
def _use_cholesky(u, params):
  ...
  y = lsp_linalg.cholesky(x)
  z = lsp_linalg.solve_triangular(y.T, u.T, lower=True).conj()
  z = (lsp_linalg.solve_triangular(y, z)).T.conj()
  e = b / c
  u = e * u + (a - e) * z
  return u
...
def qdwh(
    x: jnp.ndarray,
    is_symmetric: bool,
    max_iterations: int = 10) -> Tuple[jnp.ndarray, jnp.ndarray, int, bool]:
  """QR-based dynamically weighted Halley iteration for polar decomposition.
